...
const DEMO_SHORT_URL = "https://go.gov.sg/omedpgo";
...
function StepLabel({ n, label }: { n: number; label: string }) {
  return (
    <div className="mb-5 flex items-center gap-3">
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-ogp-blue-600 text-[12px] font-bold text-white">
        {n}
      </div>
    ...
    <>
      <div className="mb-4">
        ...
        <p className="text-[13px] leading-5 text-txt-black-500">
          Get a feel for how to ask and answer a question
        </p>
      </div>
      <div className="space-y-4">
        {groupedQuestions.map(({ group, questions }) => (
          <div key={group}>
            <p className="mb-1.5 px-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-txt-black-400">
                  ...
                  <button
                    ...
                    className={clx(
                      ...
                      activeSample === q.question
                        ? "border-otl-ogp-blue-200 bg-bg-ogp-blue-50 text-txt-black-900"
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-otl-gray-200 bg-bg-white">
      ...
      {result.rows.length === 0 ? (
        <div className="bg-bg-white px-4 py-10 text-center text-[13px] text-txt-black-400">
...
export default function QueryBuilderDashboard() {
  ...
  const [shareCopyState, setShareCopyState] = useState<"idle" | "copied">(
    "idle",
  );
  ...
  const encodedSharePath = useMemo(() => {
    const trimmedQuery = activeQueryText.trim();
    if (!trimmedQuery) return "";

    const query = encodeQuery(activeQueryText);
    return `/?query=${query}`;
  }, [activeQueryText]);
  const encodedShareUrl = encodedSharePath
    ? `${shareOrigin}${encodedSharePath}`
    : "";
  ...
  const handleCopyShareLink = useCallback(async () => {
    if (!encodedShareUrl) return;
    await navigator.clipboard.writeText(encodedShareUrl);
    setShareCopyState("copied");
    setTimeout(() => setShareCopyState("idle"), 2000);
  }, [encodedShareUrl]);

  const handleGenerateShortLink = useCallback(async () => {
    setShortShareUrl(DEMO_SHORT_URL);
    await navigator.clipboard.writeText(DEMO_SHORT_URL);
  ...
  }, [db, initializing, queryText, runQuery, running]);
  ...
  const status = (() => {
    if (dbError)
      return { text: `DuckDB error: ${dbError}`, cls: "text-ogp-blue-600" };
    if (initializing)
  ...
  return (
    <div className="sm:px-4.5 px-3 md:px-6">
        ...
        <main className="min-w-0 flex-1 px-2 pb-24 pt-8 sm:px-6 lg:px-10">
          ...
          <section className="mb-12">
            ...
            <p className="mb-4 max-w-2xl text-body-sm text-txt-black-700">
              Copy the prompt we&apos;ve prepared
              for you, paste it into the AI tool of your choice, and just ask
              your question directly. It should be able to generate the code
            ...
            >
              <div className="space-y-3">
                <p className="max-w-2xl text-body-sm text-txt-black-700">
                  ...
                  inspect. The underlying data is derived from open election
                  datasets sourced from{" "}
                  <Link
                    href="https://data.gov.sg/datasets?agencies=Elections+Department+(ELD)&resultId=1531"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-medium text-txt-ogp-blue underline underline-offset-2"
                  >
                    data.gov.sg
                  </Link>
                  .
                ...
                <div className="flex">
                  <button
                    ...
                    className={clx(
                      ...
                      "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ogp-blue-600/30",
                      activeSource === "workspace"
                        ? "border-otl-ogp-blue-200 bg-bg-ogp-blue-50"
                        : "border-otl-gray-200 bg-bg-white hover:border-otl-ogp-blue-200 hover:bg-bg-black-50",
                    )}
                  >
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 border-t border-otl-gray-200 px-3 py-2">
                  <p className="text-[12px] text-txt-black-500">
                    Run your query directly in the browser with{" "}
                  ...
                  <button
                    ...
                    className={clx(
                      ...
                      "bg-ogp-blue-600 text-white hover:bg-ogp-blue-700",
          ...
          <section className="mb-12">
            ...
            <p className="mb-4 max-w-2xl text-body-sm text-txt-black-700">
              This URL fully encodes your query, so anyone with the link can
              open it later with the same query ready to run. Your query is not
              stored.
            </p>
            <div className="mb-4 max-w-2xl rounded-lg border border-otl-gray-200 bg-bg-washed px-3 py-2">
              <div className="mb-1 flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  disabled={!encodedShareUrl}
                  title={
                    shareCopyState === "copied" ? "Copied!" : "Copy encoded URL"
                  }
              ...
              <p className="break-all font-mono text-[12px] leading-5 text-txt-black-700">
                {encodedShareUrl || "Write a query to generate an encoded URL."}
              </p>
            </div>
            <p className="mb-4 max-w-2xl text-body-sm text-txt-black-700">
              If you don&apos;t mind us storing your query, you can generate a
              short link with{" "}
              <Link
                href="https://go.gov.sg/"
                target="_blank"
                rel="noopener noreferrer"
                className="font-medium text-txt-ogp-blue underline underline-offset-2"
              >
            ...
            {shortShareUrl ? (
              <div className="mb-4 max-w-2xl rounded-lg border border-otl-gray-200 bg-bg-washed px-3 py-2">
                <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-txt-black-500">
                  Short link (note: dummy for demo)
                </p>
                <p className="break-all font-mono text-[12px] leading-5 text-txt-black-700">
                  {shortShareUrl}
                </p>
